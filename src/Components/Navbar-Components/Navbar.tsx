...
import NavbarMenuButttons from "./NavBarMenuButtons";
import DarkModeButton from "./DarkModebutton";
import { Link } from "react-router-dom";
import SearchBarButton from "../Search-Components/SearchBarButton";
import SearchBar from "../Search-Components/SearchBar";
import { Dialog, Transition } from "@headlessui/react";

const Navbar = () => {
  // State variable for menu's open/close status in mobile view.
  let [ismenuOpen, setIsmenuOpen] = useState(false);

  // State variable controlling visibility of a modal or overlay.
  let [isOpen, setIsOpen] = useState(false);

  // Toggles the value of ismenuOpen between true and false.
  const openMobileMenu = () => {
    setIsmenuOpen(!ismenuOpen);
  };

  // Sets the value of isOpen to false, closing the modal or overlay.
  function closeModal() {
    setIsOpen(false);
  }

  // Sets the value of isOpen to true, opening the modal or overlay.
  function openModal() {
    setIsOpen(true);
  }
      ...
      <nav className="bg-white border-gray-200 dark:bg-gray-900 p-4">
        <div className="max-w-screen-xl flex gap-2  items-center justify-between mx-auto mb-4">
          ...
          <div className={"items-center justify-between w-full md:w-auto mr-auto md:ml-10 hidden md:flex"} id="navbar-search">
            ...
            <NavbarMenuButttons onMenuToggle={() => {}} />
          </div>

          {/* SearchBarButton */}
          <SearchBarButton clicker={openModal} />

          {/* DARK MODE BUTTON AND MOBILE BUTTON */}
        ...
        <div className={"md:hidden" + (ismenuOpen ? "" : " hidden")} id="navbar-Mobile">
          <NavbarMenuButttons onMenuToggle={openMobileMenu} />
        </div>
